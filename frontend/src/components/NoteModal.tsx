...
function NoteModal() {
  ...
  return (
    <div className="min-h-dvh h-full flex justify-center items-center">
      <div className="w-2/3 h-full bg-lightpurple">
        <form
          method="post"
          onSubmit={handleSubmit(onSubmit)}
          className="flex flex-col my-4 mx-4"
        >
          <button className="self-end mr-1">X</button>
          <input
            defaultValue="Title.."
            {...register("title")}
            className="w-fit"
          ...
          <textarea
            {...register("content")}
            className="h-60 mb-5 resize-none"
          />
          <button type="submit" className="border w-fit p-2 self-start text-sm">
            Add note
          </button>
        </form>
      </div>
